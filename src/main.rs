...
async fn compute_permutation_argument(
    ...
    for i in 0..64 {
        let g_i = card_share_values[i] + y1;
    ...
    // Compute d_0 as h_0 * t_0 - g_0
    // TOASSERT - check if 0
    let d_0 = h_evals[0] * t_is[0].1 - g_eval_shares[0];
    d_is.push(d_0);
    ...
    for i in 1..64 {
        ...
        // d_i = t_i * h_i - alpha_i
        let tmp = evaluator.scale(&t_is[i].0, -h_evals[i]);
        let minus_d_i = evaluator.add(&tmp, &alpha_i);

        d_is.push(F::from(0) - evaluator.get_wire(&minus_d_i));
    }

    // Compute share polynomial of d(X)
    let d_share_poly = utils::interpolate_poly_over_mult_subgroup(&d_is.clone());
