...
impl Evaluator {
    ...
    pub async fn dist_sigma_proof(
        ...
        // Message 3
        let mut h_y = self.scale(wit_2_handle, gamma[0]);
        h_y = self.add(&h_y,&z2);
    ...
    pub async fn local_verify_sigma_proof(
        &mut self,
        C: &G1, D_batch: &G1, a1: &G1,
        g: &G1, c_1: &G1, a2: &G1,
        e_batch: &Gt, c2_batch: &Gt, a3: &Gt, a4: &Gt,
        x: F,
        y: F
    ) -> bool {
        ...
        let lhs = C.mul(x);
        let rhs = D_batch.mul(gamma[0]).add(a1);
        if ! lhs.eq(&rhs) {
            return false
        }
        ...
        let lhs = g.mul(y);
        let rhs = c_1.mul(gamma[0]).add(a2);
        if ! lhs.eq(&rhs) {
            return false
        }

        // Verify statement 3 : e_batch^x * g^y = c2_batch^gamma * a4 * a3
        let lhs = e_batch.mul(x).add(Gt::generator().mul(y));
        let rhs = c2_batch.mul(gamma[0]).add(a4).add(a3);
        if ! lhs.eq(&rhs) {
            return false
        }  
