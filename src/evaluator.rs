...
impl Evaluator {
    ...
    pub async fn dist_sigma_proof(
        ...
        let gamma = vec![F::from(42);1];

        utils::fs_hash(vec![&a1_bytes, &a2_bytes, &a3_bytes, &a4_bytes], 1);

        // Message 3
        let mut h_y = self.scale(&wit_2_handle.clone(), gamma[0]);
        h_y = self.add(&h_y,&z2);
        let y = self.output_wire(&h_y).await;

        // x = gamma * sum_i (lin_comb_ran[i] * wit_1_handles[i]) + z2
        let mut h_x = self.scale(&wit_1_handles[0], lin_comb_ran[0]);
...
pub async fn test_sigma(evaluator: &mut Evaluator) {
    ...
    let check = evaluator.local_verify_sigma_proof(
        &G1::generator(), 
        &d_batch, 
        &G1::generator(), 
        &c_1, 
